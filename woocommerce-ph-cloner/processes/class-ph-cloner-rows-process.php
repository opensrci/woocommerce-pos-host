...
class PH_Cloner_Rows_Process extends PH_Cloner_Process {
	...
	protected function task( $item ) {
		...
		if ( empty( $row ) ) {
			ph_cloner_log()->log( "SKIPPING row *$row_num* in *$source_table* because content was empty" );
		...
		if ( ! $do_copy_row ) {
			ph_cloner_log()->log( [ "SKIPPING row in *$source_table* because do_copy_row was false:", $row ] );
			return parent::task( $item );
		}

		// Perform replacements.
		$replaced_in_row   = 0;
		$search_replace    = ph_cloner_request()->get_search_replace( $source_id, $target_id );
		...
		if ( $do_search_replace ) {
			foreach ( $row as $field => $value ) {
				$replaced_in_column = ph_recursive_search_replace(
					...
					ph_cloner_request()->get( 'case_sensitive', false )
				);
				$replaced_in_row   += $replaced_in_column;
				$row[ $field ]      = $value;
			}
			if ( $replaced_in_row > 0 ) {
				ph_cloner_log()->log( "PERFORMED *$replaced_in_row* replacements in *$target_table*" );
				ph_cloner()->report->increment_report( '_replacements', $replaced_in_row );
			}
		} else {
			ph_cloner_log()->log( [ "SKIPPING row replacements in *$source_table* because do_copy_row was false:", $row ] );
		}
	...
	protected function get_row( $source_id, $source_table, $row_num ) {
		$source_prefix  = is_multisite() ? ph_cloner()->db->get_blog_prefix( $source_id ) : ph_cloner()->db->prefix;
		...
		if ( ! isset( $this->primary_keys[ $source_table ] ) ) {
			...
			$key_data = ph_cloner()->db->get_results( "SHOW KEYS FROM $source_table WHERE Key_name = 'PRIMARY'" );
			$this->primary_keys[ $source_table ] = [
				'name'  => $key_data && 1 === count( $key_data ) ? $key_data[0]->Column_name : false,
				'value' => 0,
			];
			ph_cloner_log()->log( [ "CHECKING primary key for *$source_table*", $key_data ] );
		}
		...
		if ( ! isset( $this->preloaded[ $source_table ][ $row_num ] ) ) {
			...
			$query = "SELECT $source_table.* FROM `$source_table`"
				. ' ' . apply_filters( 'ph_cloner_rows_where', $where, $source_table, $source_prefix )
				. ' ' . apply_filters( 'ph_cloner_rows_order', $order, $source_table, $source_prefix )
				. ' ' . apply_filters( 'ph_cloner_rows_limit', $limit, $source_table, $source_prefix );
			// Run it!
			ph_cloner_log()->log( [ "PRELOADING rows for *$source_table* with query:", $query ] );
			...
			ph_cloner_log()->log( 'PRELOADED *' . count( $this->preloaded[ $source_table ] ) . "* rows for *$source_table*" );
		}
		...
		if ( isset( $this->preloaded[ $source_table ][ $row_num ] ) ) {
			...
			ph_cloner()->report->add_notice( "Missing row *$row_num* in *$source_table* - could not be preloaded" );
			return [];
		}
	}
	...
	protected function insert_row( $row, $target_table ) {
		$row         = apply_filters( 'ph_cloner_insert_values', $row, $target_table );
		...
		$formats             = implode( ', ', ph_prepare_row_formats( $row, $target_table ) );
	...
	public function insert_batch() {
		ph_cloner_log()->log( "INSERTING $this->rows_count rows into $this->current_table" );
		...
		foreach ( $inserts as $query ) {
			...
			if ( ! empty( ph_cloner()->db->last_error ) ) {
				if ( false !== strpos( ph_cloner()->db->last_error, 'Duplicate entry' ) ) {
					ph_cloner()->report->add_notice( ph_cloner()->db->last_error . ' for table ' . $this->current_table );
					ph_cloner_log()->log( [ 'DUPLICATE entry for query:', $query ] );
					ph_cloner()->db->last_error = '';
				} else {
	...
	protected function after_handle() {
		...
		ph_cloner_log()->log( [ 'SAVING primary key data for rows process:', $this->primary_keys ] );
	...
	protected function is_query_maxed() {
		...
		$rows_per_query      = apply_filters( 'ph_cloner_rows_per_query', $this->rows_per_query, $this->identifier );
