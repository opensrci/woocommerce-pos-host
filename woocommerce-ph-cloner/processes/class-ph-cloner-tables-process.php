 ...
 */
class PH_Cloner_Tables_Process extends PH_Cloner_Process {
	...
	public function __construct() {
		parent::__construct();
		$this->report_label = __( 'Tables', 'ph-cloner-site-copier' );
	}
	...
	protected function task( $item ) {
		...
		$rows_process  = ph_cloner()->get_process( 'rows' );
		$source_prefix = ph_cloner()->db->get_blog_prefix( $item['source_id'] );
		$target_prefix = ph_cloner()->db->get_blog_prefix( $item['target_id'] );
		...
		if ( apply_filters( 'ph_cloner_do_drop_target_table', true, $target_table ) ) {
			$drop_query  = "DROP TABLE IF EXISTS `$target_table`";
			$drop_result = ph_cloner()->db->query( $drop_query );
			ph_cloner_log()->handle_any_db_errors();
			$create_query = ph_sql_create_table_query( $source_table, $target_table, $source_prefix, $target_prefix );
			// If it was a view, the create query will be returned empty, so skip.
			if ( empty( $create_query ) ) {
				return false;
			}
			$create_result = ph_cloner()->db->query( $create_query );
			ph_cloner_log()->handle_any_db_errors();
			...
			if ( false === $create_result ) {
				ph_cloner_log()->log( "SKIPPING TABLE *$source_table*. Could not create table." );
				return false;
			}
		}
		...
		$where      = apply_filters( 'ph_cloner_rows_where', 'WHERE 1=1', $source_table, $source_prefix );
		$count_rows = ph_cloner()->db->get_var( "SELECT COUNT(*) rows_qty FROM `$source_table` $where" );
		ph_cloner_log()->log( [ "SELECTED $count_rows with query:", "SELECT COUNT(*) rows_qty FROM `$source_table` $where" ] );
		if ( $count_rows > 0 ) {
			...
			for ( $i = $next_row; $i < $count_rows; $i++ ) {
				...
				if ( $i && 0 === $i % 5000 ) {
					if ( $this->time_exceeded() || $this->memory_exceeded() ) {
						...
						ph_cloner_log()->log( "STOPPING partially queued table at row $i due to resource limits" );
						$item['next_row'] = $i + 1;
						return $item;
					}
				}
			...
			$rows_process->save();
			ph_cloner_log()->log( "QUEUEING *$count_rows* rows from *$source_table* to *$target_table*" );
		} else {
			ph_cloner_log()->log( "SKIPPING TABLE *$source_table*, 0 rows found." );
		}
