 ...
 */
abstract class PH_Cloner_Process extends WP_Background_Process {
	...
	public function push_to_queue( $data ) {
		...
		if ( count( $this->data ) >= $max_batch ) {
			ph_cloner_log()->log( "REACHING max batch of *$max_batch* in queue - autosaving and starting new" );
			$this->save();
		}
	...
	public function save() {
		...
		if ( ! empty( $this->data ) ) {
			...
			ph_cloner_log()->log( "SAVING batch for $this->identifier with " . count( $this->data ) . ' items.' );
	...
	public function dispatch() {
		if ( $this->is_queue_empty() ) {
			...
			ph_cloner_log()->log( "DISPATCHING *$this->identifier* to " . add_query_arg( $this->get_query_args(), $this->get_query_url() ) );
			$response = parent::dispatch();
			ph_cloner_log()->log( [ 'RECEIVED response to dispatch', $response ] );
		}
		return $this;
	}
	...
	public function is_process_running() {
		ph_cloner()->process_manager->doing_cloning();
		$lock_value = ph_cloner()->db->get_var(
		...
		if ( isset( $_REQUEST['force_process'] ) ) {
			ph_cloner_log()->log( "FORCING manual run for *$this->identifier* - overriding any existing instances" );
			if ( $lock_value ) {
				ph_cloner_log()->log( "FOUND existing lock $lock_value so deleting it" );
				ph_cloner()->db->query(
					ph_prepare_option_query(
			...
			ph_cloner_log()->log( "CHECKING for running *$this->identifier* - none found" );
			return false;
		} elseif ( $lock_value === $this->lock_id ) {
			ph_cloner_log()->log( "CHECKING for running *$this->identifier* - the running process is the current one ($this->lock_id)" );
			return false;
		} else {
			ph_cloner_log()->log( "CHECKING for running *$this->identifier* - found $lock_value" );
			return true;
		}
	}
	...
	protected function lock_process() {
		...
		// Save the lock to db.
		ph_cloner_log()->log( "LOCKING *$this->identifier* with id $this->lock_id" );
		ph_cloner()->db->query(
			ph_prepare_option_query(
		...
		if ( $this->get_lock() !== $this->lock_id ) {
			ph_cloner_log()->log( "DETECTED simultaneous *$this->identifier* - ending" );
			exit;
		}
	}
	...
	public function handle() {
		...
		ph_cloner()->process_manager->doing_cloning();
		ph_cloner_log()->log_break();
		ph_cloner_log()->log( "HANDLING <b>$this->action</b> async request" );
		ph_cloner_log()->log( [ 'DISPATCHED from:', isset( $_REQUEST['ajax'] ) ? 'client' : 'server' ] );
	...
	protected function task( $item ) {
		...
		$progress_update_interval = apply_filters( 'ph_cloner_progress_update_interval', 5, $this->identifier );
	...
	protected function complete() {
		ph_cloner_log()->log( "COMPLETING *$this->identifier*" );
		parent::complete();
		...
		ph_cloner()->process_manager->maybe_finish();
	}
	...
	public function cancel() {
		...
		ph_cloner_log()->log( "ENDING $this->action background process and clearing data." );
	}
	...
	protected function time_exceeded() {
		...
		if ( $exceeded ) {
			ph_cloner_log()->log( "EXCEEDED TIME for $this->identifier" );
		}
	...
	protected function memory_exceeded() {
		...
		if ( $exceeded ) {
			ph_cloner_log()->log( "EXCEEDED MEMORY for $this->identifier with usage of " . size_format( memory_get_usage( true ) ) );
		}
		return $exceeded;
	}
	...
	private function get_batches() {
		...
		$progress_rows = ph_cloner()->db->get_results(
			ph_prepare_option_query(
	...
	public function update_batch_progress( $batch_key, $data ) {
		...
		if ( empty( $data ) ) {
			...
			ph_cloner_log()->log( "DELETING progress for <b>$batch_key</b>" );
		} else {
			...
			ph_cloner_log()->log( [ 'UPDATING ' . $progress_key, $new_progress ] );
		}
	}
