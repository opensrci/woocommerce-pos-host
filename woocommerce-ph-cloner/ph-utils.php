 ...
 * @return string
 */
function ph_sql_create_table_query( $source_table, $target_table, $source_prefix, $target_prefix ) {
	...
	if ( preg_match( $view, $query ) ) {
		...
		if ( ! apply_filters( 'ph_cloner_skip_views', false ) ) {
			...
			ph_cloner()->process_manager->add_finish_query( $view_query, 200 );
		}
		return '';
	}
	...
	if ( ! apply_filters( 'ph_cloner_skip_constraints', false ) ) {
		foreach ( $constraint_matches[1] as $constraint_def ) {
			...
			ph_cloner()->process_manager->add_finish_query( "ALTER TABLE `$constraint_table` ADD $constraint_def;", 100 );
		}
	}
