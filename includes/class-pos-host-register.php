 ...
 * POS_HOST_Register.
 */
class POS_HOST_Register extends WC_Data {
	 ...
	 * @var string
	 */
	protected $object_type = 'pos_host_register';
	 ...
	 * @var string
	 */
	protected $post_type = 'pos_host_register';
	 ...
	 * @var string
	 */
	protected $cache_group = 'pos_host_registers';
	...
	public function __construct( $data = '' ) {
		...
		if ( $data instanceof POS_HOST_Register ) {
		...
		$register = apply_filters( 'pos_host_register_get_data', false, $data, $this );
		...
		if ( is_string( $data ) && 'pos_host_register' === get_post_type( absint( $data ) ) ) {
			$this->set_id( absint( $data ) );
		} elseif ( is_string( $data ) ) {
			$post = get_page_by_path( $data, OBJECT, 'pos_host_register' );

			if ( $post && isset( $post->ID ) && 'pos_host_register' === get_post_type( $post->ID ) ) {
				$this->set_id( $post->ID );
			} else {
				$this->set_object_read( true );
			}
		} elseif ( is_int( $data ) && 'pos_host_register' === get_post_type( $data ) ) {
			$this->set_id( $data );
		} else {
			$this->set_object_read( true );
		}
	...
	protected function read_object_from_database() {
		$this->data_store = WC_Data_Store::load( 'pos_host_register' );
	...
	/**
	 * Developers can programmatically return registers. This function will read those values into our POS_HOST_Register class.
	 *
	 * @param string $slug     Register slug.
