 ...
 * @since 0.0.1
 *
 * @param int|POS_HOST_Grid $id Grid ID or object.
 *
 * @throws Exception If grid cannot be read/found and $data parameter of POS_HOST_Grid class constructor is set.
 * @return POS_HOST_Grid|null
 */
function pos_host_get_grid( $id ) {
	$grid = new POS_HOST_Grid( (int) $id );
 ...
 * @return array
 */
function pos_host_get_tile_grids( $item_id, $type = 'product', $associative = false ) {
	global $wpdb;
	...
	$get_grids = $wpdb->get_results(
		$wpdb->prepare(
			...
			FROM {$wpdb->prefix}pos_host_grid_tiles as gt
 ...
 */
function pos_host_get_grid_tile_by_item_id( $grid_id, $item_id, $type = 'product' ) {
	global $wpdb;

	$result = $wpdb->get_var(
		$wpdb->prepare(
			"SELECT id FROM {$wpdb->prefix}pos_host_grid_tiles WHERE type = %s AND item_id = %d AND grid_id = %d",
			$type,
			$item_id,
			$grid_id
		)
 ...
 */
function pos_host_reorder_grid_tiles( $grid_id, $current_position, $new_position ) {
	global $wpdb;
	...
	$wpdb->query(
		$wpdb->prepare(
			"UPDATE {$wpdb->prefix}pos_host_grid_tiles SET display_order = 0 WHERE display_order = %d AND grid_id = %d",
			$current_position,
			$grid_id
		)
	);
	...
	if ( 'down' === $move ) {
		$wpdb->query(
			$wpdb->prepare(
				"UPDATE {$wpdb->prefix}pos_host_grid_tiles SET display_order = (display_order - 1) WHERE display_order > %d AND display_order <= %d AND grid_id = %d",
				$current_position,
				$new_position,
				$grid_id
			)
	...
	$result = $wpdb->query(
		$wpdb->prepare(
			"UPDATE {$wpdb->prefix}pos_host_grid_tiles SET display_order = %d WHERE display_order = 0 AND grid_id = %d",
			$new_position,
			$grid_id
		)
	);
 ...
 */
function pos_host_is_in_grid( $grid_id, $item_id, $type = 'product' ) {
	global $wpdb;

	$results = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT id FROM {$wpdb->prefix}pos_host_grid_tiles
...
function pos_host_grid_thumbnail( $thumbnail_id, $size = 'thumbnail' ) {
