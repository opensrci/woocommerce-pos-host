 ...
 * POS_HOST_Outlet.
 */
class POS_HOST_Outlet extends WC_Data {
	 ...
	 * @var string
	 */
	protected $object_type = 'pos_host_outlet';
	 ...
	 * @var string
	 */
	protected $post_type = 'pos_host_outlet';
	 ...
	 * @var string
	 */
	protected $cache_group = 'pos_host_outlets';
	...
	public function __construct( $data = '' ) {
		...
		if ( $data instanceof POS_HOST_Outlet ) {
			$this->set_id( absint( $data->get_id() ) );
			$this->read_object_from_database();
			return;
		}

		// This filter allows custom outlet objects to be created on the fly.
		$outlet = apply_filters( 'pos_host_get_pos_host_outlet_data', false, $data, $this );
		...
		if ( is_string( $data ) && 'pos_host_outlet' === get_post_type( absint( $data ) ) ) {
			$this->set_id( absint( $data ) );
		} elseif ( is_string( $data ) ) {
			$post = get_page_by_path( $data, OBJECT, 'pos_host_outlet' );

			if ( $post && isset( $post->ID ) && 'pos_host_outlet' === get_post_type( $post->ID ) ) {
				$this->set_id( $post->ID );
			} else {
				$this->set_object_read( true );
			}
		} elseif ( is_int( $data ) && 'pos_host_outlet' === get_post_type( $data ) ) {
			$this->set_id( $data );
		} else {
			$this->set_object_read( true );
		}
	...
	/**
	 * Developers can programmatically return outlets. This function will read those values into our POS_HOST_Outlet class.
	 *
	 * @param string $slug   Outlet slug.
