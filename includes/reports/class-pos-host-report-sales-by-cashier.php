 ...
 */
class POS_HOST_Report_Sales_By_Cashier extends WC_Admin_Report {
	...
	public function __construct() {
		if ( isset( $_GET['cashier_ids'] ) && is_array( $_GET['cashier_ids'] ) ) {
			$this->cashier_ids = array_map( 'absint', $_GET['cashier_ids'] );
		} elseif ( isset( $_GET['cashier_ids'] ) ) {
			$this->cashier_ids = array( absint( $_GET['cashier_ids'] ) );
		}
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s gross sales', 'woocommerce-pos-host' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s net sales', 'woocommerce-pos-host' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
	...
	public function current_filters() {
		...
		echo '<p><strong>' . esc_html( implode( ', ', $this->cashier_ids_titles ) ) . '</strong></p>';
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'cashier_ids' ) ) . '">' . esc_html__( 'Reset', 'woocommerce-pos-host' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php esc_html_e( 'Search Cashier', 'woocommerce-pos-host' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<select class="wc-product-search" style="width:203px;" name="cashier_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a cashier&hellip;', 'woocommerce-pos-host' ); ?>" data-action="pos_host_json_search_cashier"> </select>
		...
		<h4 class="section_title"><span><?php esc_html_e( 'Top cashiers', 'woocommerce-pos-host' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $cashier ) {
						...
						echo '<tr class="' . esc_attr( in_array( $cashier->cashier_id, $this->cashier_ids ) ? 'active' : '' ) . '">
							<td class="count">' . esc_html( $cashier->count ) . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'cashier_ids', $cashier->cashier_id ) ) . '">' . esc_html( $user_name ) . '</a></td>
							<td class="sparkline">' . wp_kses_post( $this->event_sales_sparkline( $cashier->cashier_id, 7, 'count' ) ) . '</td>
						</tr>';
					}
				} else {
					echo '<tr><td colspan="3">' . esc_html__( 'No cashiers found in range', 'woocommerce-pos-host' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
			...
			<table cellspacing="0">
				...
				if ( $top_earners ) {
					foreach ( $top_earners as $cashier ) {
						...
						echo '<tr class="' . esc_attr( in_array( $cashier->cashier_id, $this->cashier_ids ) ? 'active' : '' ) . '">
							<td class="count">' . wp_kses_post( wc_price( $cashier->order_total ) ) . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'cashier_ids', $cashier->cashier_id ) ) . '">' . esc_html( $user_name ) . '</a></td>
							<td class="sparkline">' . wp_kses_post( $this->event_sales_sparkline( $cashier->cashier_id, 7, 'sales' ) ) . '</td>
						</tr>';
					}
				} else {
					echo '<tr><td colspan="3">' . esc_html__( 'No cashiers found in range', 'woocommerce-pos-host' ) . '</td></tr>';
	...
	public function get_main_chart() {
		...
		if ( ! $this->cashier_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php esc_html_e( '&larr; Choose a cashier to view stats', 'woocommerce-pos-host' ); ?></p>
			</div>
			<?php
		} else {
		...
		<script type="text/javascript">
			...
			jQuery(function(){
				...
				var drawGraph = function( highlight ) {
					...
					if ( 'undefined' !==  highlight && series[ highlight ] ) {
					...
					main_chart = jQuery.plot(
						...
						series,
						{
							...
							xaxes: [ {
								...
								mode: "time",
								timeformat: "<?php 'day' === $this->chart_groupby ? '%d %b' : '%b'; ?>",
