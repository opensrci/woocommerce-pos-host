...
/**
 * POS_HOST_Report_Sales_By_Session.
 */
class POS_HOST_Report_Sales_By_Session extends WC_Admin_Report {

	public $chart_colours      = array();
	public $session_ids        = array();
	public $session_ids_titles = array();
	private $report_data;
	...
	public function __construct() {
		if ( isset( $_GET['session_ids'] ) && is_array( $_GET['session_ids'] ) ) {
			$this->session_ids = array_map( 'absint', $_GET['session_ids'] );
		} elseif ( isset( $_GET['session_ids'] ) ) {
			$this->session_ids = array( absint( $_GET['session_ids'] ) );
		}
	}
		...
		$this->report_data->orders = (array) $this->get_order_report_data(
			array(
				...
				'where_meta'          => array(
					...
					array(
						'meta_key'   => array( 'pos_host_id_session' ),
						'meta_value' => $this->session_ids,
						'operator'   => 'IN',
					),
				),
	...
	public function get_chart_legend() {
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s gross sales from this session', 'woocommerce-pos-host' ), '<strong>' . wc_price( $data->total_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s net sales from this session', 'woocommerce-pos-host' ), '<strong>' . wc_price( $data->net_sales ) . '</strong>' ),
		...
		$legend[] = array(
			...
			'title'            => sprintf( __( '%s sales from this session for saved orders', 'woocommerce-pos-host' ), '<strong>' . wc_price( $data->saved_sales ) . '</strong>' ),
	...
	public function current_filters() {
		...
		$this->session_ids_titles = array();

		foreach ( $this->session_ids as $session_id ) {

			$session = pos_host_get_session( absint( $session_id ) );

			if ( $session ) {
				$this->session_ids_titles[] = $session->get_name();
			} else {
				$this->session_ids_titles[] = '#' . $session_id;
			}
		}

		echo '<p><strong>' . esc_html( implode( ', ', $this->session_ids_titles ) ) . '</strong></p>';
		echo '<p><a class="button" href="' . esc_url( remove_query_arg( 'session_ids' ) ) . '">' . esc_html__( 'Reset', 'woocommerce-pos-host' ) . '</a></p>';
	}
	...
	public function products_widget() {
		?>
		<h4 class="section_title"><span><?php esc_html_e( 'Session Search', 'woocommerce-pos-host' ); ?></span></h4>
		<div class="section">
			<form method="GET">
				<div>
					<select class="wc-product-search" style="width:203px;" name="session_ids[]" data-placeholder="<?php esc_attr_e( 'Search for a session&hellip;', 'woocommerce-pos-host' ); ?>" data-action="pos_host_json_search_sessions"></select>
		...
		<h4 class="section_title"><span><?php esc_html_e( 'Top Sessions', 'woocommerce-pos-host' ); ?></span></h4>
		<div class="section">
			<table cellspacing="0">
				<?php
				$top_sellers = $this->get_order_report_data(
					array(
						'data'         => array(
							...
							'pos_host_id_session' => array(
								'type'     => 'meta',
								'function' => 'COUNT',
								'name'     => 'count, meta_pos_host_id_session.meta_value as session_id',
							),
						),
						'where_meta'   => array(
							array(
				...
				if ( $top_sellers ) {
					foreach ( $top_sellers as $session ) {
						$reg      = pos_host_get_session( absint( $session->session_id ) );
						...
						echo '<tr class="' . esc_attr( in_array( $session->session_id, $this->session_ids ) ? 'active' : '' ) . '">
							<td class="count">' . esc_html( $session->count ) . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'session_ids', $session->session_id ) ) . '">' . esc_html( $reg_name ) . '</a></td>
							<td class="sparkline">' . wp_kses_post( $this->event_sales_sparkline( $session->session_id, 7, 'count' ) ) . '</td>
						</tr>';
					}
				} else {
					echo '<tr><td colspan="3">' . esc_html__( 'No sessions found in range', 'woocommerce-pos-host' ) . '</td></tr>';
				}
				?>
			</table>
		</div>
		...
		<div class="section">
			<table cellspacing="0">
				...
				if ( $top_earners ) {
					foreach ( $top_earners as $session ) {
						$reg      = pos_host_get_session( absint( $session->session_id ) );
						$reg_name = $reg->get_name();
						...
						echo '<tr class="' . esc_attr( in_array( $session->session_id, $this->session_ids ) ? 'active' : '' ) . '">
							<td class="count">' . wp_kses_post( wc_price( $session->order_total ) ) . '</td>
							<td class="name"><a href="' . esc_url( add_query_arg( 'session_ids', $session->session_id ) ) . '">' . esc_html( $title ) . '</a></td>
							<td class="sparkline">' . wp_kses_post( $this->event_sales_sparkline( $session->session_id, 7, 'sales' ) ) . '</td>
						</tr>';
					}
				} else {
					echo '<tr><td colspan="3">' . esc_html__( 'No sessions found in range', 'woocommerce-pos-host' ) . '</td></tr>';
	...
	public function get_main_chart() {
		...
		if ( ! $this->session_ids ) {
			...
			<div class="chart-container">
				<p class="chart-prompt"><?php esc_html_e( '&larr; Choose a session to view stats', 'woocommerce-pos-host' ); ?></p>
			</div>
			<?php
		} else {
