 ...
 */
class POS_HOST_Admin_List_Table_Outlets extends WC_Admin_List_Table {
	 ...
	 * @var string
	 */
	protected $list_table_type = 'pos_host_outlet';
	...
	protected function get_row_actions( $actions, $post ) {
		...
		if ( pos_host_is_default_outlet( $this->object->get_id() ) ) {
			unset( $actions['trash'] );
		}
	...
	protected function prepare_row_data( $post_id ) {
		...
		if ( empty( $this->object ) || $this->object->get_id() !== $post_id ) {
			$the_outlet   = pos_host_get_outlet( $post_id );
	...
	protected function render_outlet_column() {
		 ...
		 */
		if ( pos_host_is_default_outlet( $this->object->get_id() ) ) {
			$output .= wc_help_tip( __( 'This is the default outlet and it cannot be deleted.', 'woocommerce-pos-host' ) );
	...
	protected function render_registers_column() {
		...
		$count = $wpdb->get_var(
			$wpdb->prepare(
				...
				ON p.ID = pm.post_id
				WHERE p.post_type = 'pos_host_register' AND pm.meta_key = 'outlet' AND pm.meta_value = %d
				",
				$this->object->get_id()
			)
		);
	...
	public function order_by_registers_desc_post_clauses( $args ) {
		...
		$args['orderby'] .= " WHERE registers_lookup.post_type = 'pos_host_register' AND registers_postmeta_lookup.meta_key = 'outlet' AND registers_postmeta_lookup.meta_value = {$wpdb->posts}.ID) DESC";
	...
	public function order_by_registers_asc_post_clauses( $args ) {
		...
		$args['orderby']  = " (SELECT COUNT(*) FROM {$wpdb->posts} registers_lookup";
		$args['orderby'] .= " LEFT JOIN {$wpdb->postmeta} registers_postmeta_lookup ON registers_lookup.ID = registers_postmeta_lookup.post_id";
		$args['orderby'] .= " WHERE registers_lookup.post_type = 'pos_host_register' AND registers_postmeta_lookup.meta_key = 'outlet' AND registers_postmeta_lookup.meta_value = {$wpdb->posts}.ID) ASC";
