 ...
 */
class POS_HOST_Admin_List_Table_Grids extends WC_Admin_List_Table {
	 ...
	 * @var string
	 */
	protected $list_table_type = 'pos_host_grid';
	...
	public function define_columns( $columns ) {
		...
		$show_columns['grid']  = esc_html__( 'Grid', 'woocommerce-pos-host' );
		$show_columns['tiles'] = esc_html__( 'Tiles', 'woocommerce-pos-host' );
	...
	protected function prepare_row_data( $post_id ) {
		...
		if ( empty( $this->object ) || $this->object->get_id() !== $post_id ) {
			$the_grid     = pos_host_get_grid( $post_id );
	...
	protected function render_blank_state() {
		...
		echo '<a class="woocommerce-BlankState-cta button-primary button" href="' . esc_url( admin_url( 'post-new.php?post_type=pos_host_grid' ) ) . '">' . esc_html__( 'Create your first product grid', 'woocommerce-pos-host' ) . '</a>';
		echo '<a class="woocommerce-BlankState-cta button" target="_blank" href="#">' . esc_html__( 'Learn more about grids', 'woocommerce-pos-host' ) . '</a>';
		echo '</div>';
	}
	...
	protected function render_tiles_column() {
		...
		$count = $wpdb->get_var(
			$wpdb->prepare(
				"SELECT COUNT(*)
				FROM {$wpdb->prefix}pos_host_grid_tiles
				WHERE grid_id = %d
				",
				$this->object->get_id()
			)
	...
	public function order_by_tiles_desc_post_clauses( $args ) {
		...
		$args['orderby']  = " (SELECT COUNT(*) FROM {$wpdb->prefix}pos_host_grid_tiles tiles_lookup";
	...
	public function order_by_tiles_asc_post_clauses( $args ) {
		...
		$args['orderby']  = " (SELECT COUNT(*) FROM {$wpdb->prefix}pos_host_grid_tiles";
		$args['orderby'] .= " WHERE {$wpdb->prefix}pos_host_grid_tiles.grid_id = {$wpdb->posts}.ID) ASC";
