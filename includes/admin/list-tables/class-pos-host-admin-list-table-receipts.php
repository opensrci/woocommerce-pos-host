 ...
 */
class POS_HOST_Admin_List_Table_Receipts extends WC_Admin_List_Table {
	 ...
	 * @var string
	 */
	protected $list_table_type = 'pos_host_receipt';
	...
	protected function get_row_actions( $actions, $post ) {
		...
		if ( pos_host_is_default_receipt( $this->object->get_id() ) ) {
			unset( $actions['trash'] );
		}
	...
	public function define_columns( $columns ) {
		...
		$show_columns['receipt']          = esc_html__( 'Receipt', 'woocommerce-pos-host' );
		$show_columns['no_copies']        = esc_html__( 'Number of Copies', 'woocommerce-pos-host' );
		$show_columns['width']            = esc_html__( 'Print Width', 'woocommerce-pos-host' );
		$show_columns['show_tax_summary'] = esc_html__( 'Show Tax Summary', 'woocommerce-pos-host' );
	...
	protected function prepare_row_data( $post_id ) {
		...
		if ( empty( $this->object ) || $this->object->get_id() !== $post_id ) {
			$the_receipt  = pos_host_get_receipt( $post_id );
	...
	protected function render_receipt_column() {
		 ...
		 */
		if ( pos_host_is_default_receipt( $this->object->get_id() ) ) {
			$output .= wc_help_tip( __( 'This is the default receipt and it cannot be deleted.', 'woocommerce-pos-host' ) );
	...
	/**
	 * Render column: no_copies.
	 */
	protected function render_no_copies_column() {
		echo esc_html( $this->object->get_no_copies() );
	}
	...
	protected function render_width_column() {
		echo $this->object->get_width() ? esc_html( $this->object->get_width() ) . ' ' . esc_html_x( 'mm', 'Millimeter', 'woocommerce-pos-host' ) : esc_html__( 'Dynamic', 'woocommerce-pos-host' );
	}
	...
	protected function render_show_tax_summary_column() {
		echo $this->object->get_show_tax_summary() ? esc_html__( 'Yes', 'woocommerce-pos-host' ) : esc_html__( 'No', 'woocommerce-pos-host' );
	}
}
