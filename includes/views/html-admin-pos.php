...
<html>
	...
	<body>
		...
		<script data-cfasync="false" type="text/javascript" class="pos_host_params" >
			window.pos_host_params = <?php echo wp_kses_post( WC_POS_Sell::get_js_params() ); ?>;
			...
			window.pos_host_receipt = <?php echo wp_kses_post( json_encode( WC_POS_Sell::instance()->get_receipt( $register_data['receipt'] ) ) ); ?>;
			window.pos_host_grid = <?php echo wp_kses_post( $this->get_grid() ); ?>;
			window.pos_host_wc = <?php echo wp_kses_post( WC_POS_Sell::get_js_wc_params() ); ?>;
			window.pos_host_cart = <?php echo wp_kses_post( WC_POS_Sell::get_js_cart_params() ); ?>;
			window.pos_host_i18n = <?php echo wp_kses_post( json_encode( require_once WC_POS()->plugin_path() . '/i18n/app.php' ) ); ?>;
			window.coupon_i18n = <?php echo wp_kses_post( json_encode( require_once WC_POS()->plugin_path() . '/i18n/coupon.php' ) ); ?>;
			window.pos_host_custom_product = <?php echo wp_kses_post( WC_POS_Sell::instance()->get_custom_product_params() ); ?>;
		</script>
